...
cuGraphAddHostNode(CUgraphNode *phGraphNode, CUgraph hGraph,
  ...
  if (conn == nullptr ||
      !lupine_register_host_callback(conn, nodeParams->fn, nodeParams->userData,
                                     true, &wire)) {
...
extern "C" CUresult cuLaunchHostFunc(CUstream hStream, CUhostFn fn,
  ...
  if (conn == nullptr ||
      ...
      rpc_write(conn, &wire.user_data_token, sizeof(wire.user_data_token)) <
          0 ||
      rpc_wait_for_response(conn) < 0 ||
      rpc_read(conn, &return_value, sizeof(return_value)) < 0 ||
      rpc_read_end(conn) < 0) {
  ...
  while (true) {
    ...
    if (op == 1) {
      ...
      if (lupine_read_deferred_dtoh_copies(conn) < 0 ||
          ...
          rpc_read(conn, &status, sizeof(status)) < 0 ||
          rpc_read(conn, &wire.function_token, sizeof(wire.function_token)) <
              0 ||
          rpc_read(conn, &wire.user_data_token, sizeof(wire.user_data_token)) <
              0) {
